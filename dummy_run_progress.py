...
class ProgressDemoWindow(QMainWindow):
    ...
    def log_message(self, message: str) -> None:
        ...
        self.log.append(
            f"[{QTimer.singleShot(0, lambda: None) or '--:--:--'}] {message}"
        )
    ...
    def demo_show_overlay(self) -> None:
        ...
        try:
            if self.progress_service:
                ...
                self.log_message(
                    "✓ Progress overlay shown with message: 'Initializing...'"
                )
    ...
    def demo_indeterminate_mode(self) -> None:
        ...
        try:
            if self.progress_service:
                self.progress_service.set_indeterminate()
                self.progress_service.update_message(
                    "Processing (unknown duration) - NOTICE THE THROBBER!"
                )
    ...
    def demo_combined_features(self) -> None:
        ...
        try:
            if self.progress_service:
                ...
                self.progress_service.update_message(
                    "FINAL BATCH PROCESSING - NOTICE THE VISIBILITY!"
                )
