...
class MaintenanceDialog(QDialog):
    ...
    def _on_mark_active_as_processed(self) -> None:
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                # Call the mark_active_as_processed function from operations
                from interface.operations.maintenance import mark_active_as_processed
                mark_active_as_processed(self.db_manager)
                QMessageBox.information(self, "Success", "Active folders marked as processed.")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Error: {str(e)}")
    ...
    def _on_remove_inactive(self) -> None:
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                count = 0
                for folder in self.db_manager.folders_table.find(folder_is_active="False"):
                ...
                QMessageBox.information(
                    self,
                    "Success",
                    f"Removed {count} inactive folder configurations."
                )
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Error: {str(e)}")
    ...
    def _on_clear_processed_files(self) -> None:
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.information(self, "Success", "Processed file records cleared.")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Error: {str(e)}")
        ...
        if backup_file:
            try:
                ...
                if database_import.import_interface(
                    self,
                    self.db_manager._database_path if hasattr(self.db_manager, '_database_path') else "",
