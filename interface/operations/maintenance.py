...
class MaintenanceOperations:
    ...
    def set_all_active(self) -> int:
        ...
        return self.db_manager.folders_table.count(folder_is_active="True") - active_count
    ...
    def set_all_inactive(self) -> int:
        ...
        return self.db_manager.folders_table.count(folder_is_active="False") - inactive_count
    ...
    def resend_failed_files(self) -> int:
        ...
        """
        count = 0
        failed_files = self.db_manager.processed_files.find(
            resend_flag=True
        )
        for record in failed_files:
            self.db_manager.processed_files.update(
                {"id": record["id"], "resend_flag": True},
                ["id"]
            )
            count += 1
        return count
