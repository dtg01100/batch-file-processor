...
class QtConfigSectionWidget(ConfigSectionWidget):
    ...
    def render(self, config: dict[str, Any] | None = None) -> Any:
        ...
        if factory is None:
            raise ValueError(
                f"No widget factory registered for framework: {self.framework}"
            )
    ...
    def render(self, config: dict[str, Any] | None = None) -> Any:
        ...
        for field in self.schema.fields:
            widget = factory.create_widget(
                field.field_type, field, self._content_widget
            )
            self.widgets[field.name] = widget
...
class TabbedSectionWidget(ConfigSectionWidget):
    ...
    def _render_qt(self, config: dict[str, Any] | None = None) -> Any:
        ...
        for schema in self.sections:
            section_widget = QtConfigSectionWidget(
                schema, self.framework, self.container
            )
            tab = section_widget.render(config)
            self.container.addTab(
                tab, schema.title if hasattr(schema, "title") else "Tab"
            )
