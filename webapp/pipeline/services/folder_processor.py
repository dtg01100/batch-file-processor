...
from webapp.pipeline.services.file_processor import FileProcessor
...
class FolderPipelineExecutor:
    ...
    def _filter_processed_files(
        ...
        from webapp.pipeline.services.file_filter import filter_pending_files
    ...
    def _process_file_with_pipeline(
        ...
        from webapp.pipeline.services.file_processor import FileResult
    ...
    def _record_processed_file(
        ...
        Uses the Table API (find_one / insert / update) so that the
        function works with both the production sqlite wrapper and the
        in-memory test doubles that don't expose ``raw_connection``.
    ...
    def _log_message(self, run_log: RunLog | None, message: str) -> None:
        ...
        from webapp.pipeline.file_utils import write_to_run_log
