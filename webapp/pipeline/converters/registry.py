...
class ConverterMetadata:
    ...
    Attributes:
        ...
        module_name: Full module path (e.g., "webapp.pipeline.converters.convert_to_csv")
...
def _discover_converters() -> None:
    ...
    FALLBACK_METADATA = {
        "scansheet_type_a": {
            ...
            "module_name": "webapp.pipeline.converters.convert_to_scansheet_type_a",
    ...
    for _, module_name, is_pkg in pkgutil.iter_modules([converter_path]):
        if module_name.startswith("convert_to_") and not is_pkg:
            ...
            try:
                full_module_name = f"webapp.pipeline.converters.{module_name}"
                import importlib
