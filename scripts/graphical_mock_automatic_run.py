#!/usr/bin/env python3
"""Run automatic processing graphically with mocked config, files, and database.

This script creates a temporary end-to-end environment and executes the
application's ``--graphical-automatic`` path with:
...
- GUI display showing the automatic processing

It is useful for visually testing the automatic flow without requiring real
...
class GraphicalMockRunResult:
    """Result details for a graphical mocked automatic run."""
...
def run_graphical_mock_automatic(
    base_dir: str | os.PathLike[str], keep_open: bool = True
) -> GraphicalMockRunResult:
    """Execute one graphical mocked automatic run inside ``base_dir``."""
    root = Path(base_dir)
    ...
    app = QtBatchFileSenderApp(
        ...
        version="(Graphical Mock Automatic Run)",
        database_version=CURRENT_DATABASE_VERSION,
        database_obj=db,
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a graphical mocked automatic processing smoke test",
    )
    parser.add_argument(
        "--base-dir",
        default="",
    ...
    parser.add_argument(
        "--auto-close",
        action="store_true",
        help="Close/shutdown automatically after run() returns",
    )
...
def main() -> int:
    ...
    if args.base_dir:
        ...
        base_dir = Path(tempfile.mkdtemp(prefix="batch_mock_graphical_"))

    print("Launching graphical mocked automatic mode...")
    print("Open noVNC at: http://localhost:6080/vnc.html")
    print(f"Base dir: {base_dir}")

    result = run_graphical_mock_automatic(base_dir, keep_open=not args.auto_close)
    print("Graphical mocked automatic run finished")
