...
def get_folders(db_path, active_only=False):
    ...
    cursor = conn.cursor()
    if active_only:
        cursor.execute('SELECT id, folder_name, alias, convert_to_format, folder_is_active FROM folders WHERE folder_is_active=1')
    else:
        cursor.execute('SELECT id, folder_name, alias, convert_to_format, folder_is_active FROM folders')
...
def main():
    ...
    try:
        ...
        print(f"Created fresh database at version 42")
