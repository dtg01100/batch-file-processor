...
class TestFolderConfigRoundtrip:
    ...
    def test_roundtrip_save_load(self, sample_folder_config, temp_database):
        ...
        assert (
            loaded_config.process_backend_copy
            == sample_folder_config.process_backend_copy
        )
        assert (
            loaded_config.process_backend_ftp
            == sample_folder_config.process_backend_ftp
        )
        assert (
            loaded_config.process_backend_email
            == sample_folder_config.process_backend_email
        )
        ...
        assert (
            loaded_config.copy.destination_directory
        ...
        assert (
            ...
            == sample_folder_config.edi.split_edi_include_invoices
        )
        assert (
            loaded_config.edi.split_edi_include_credits
            == sample_folder_config.edi.split_edi_include_credits
        )
        assert (
            loaded_config.edi.prepend_date_files
            == sample_folder_config.edi.prepend_date_files
        )
        assert (
            loaded_config.edi.convert_to_format
            == sample_folder_config.edi.convert_to_format
        )
        assert (
            loaded_config.edi.force_edi_validation
            == sample_folder_config.edi.force_edi_validation
        )
        assert loaded_config.edi.rename_file == sample_folder_config.edi.rename_file
        assert (
            loaded_config.edi.split_edi_filter_categories
            == sample_folder_config.edi.split_edi_filter_categories
        )
        assert (
            loaded_config.edi.split_edi_filter_mode
            == sample_folder_config.edi.split_edi_filter_mode
        )

        # Verify UPC Override
        assert loaded_config.upc_override is not None
        assert (
            loaded_config.upc_override.enabled
            == sample_folder_config.upc_override.enabled
        )
        assert (
            loaded_config.upc_override.level == sample_folder_config.upc_override.level
        )

        # Verify A-Record
        assert loaded_config.a_record_padding is not None
        assert (
            loaded_config.a_record_padding.enabled
            == sample_folder_config.a_record_padding.enabled
        )
        assert (
            loaded_config.a_record_padding.padding_text
            == sample_folder_config.a_record_padding.padding_text
        )
        ...
        assert (
            loaded_config.backend_specific.fintech_division_id
            == sample_folder_config.backend_specific.fintech_division_id
        )

        # Verify CSV
        assert loaded_config.csv is not None
        assert (
            loaded_config.csv.include_headers
            == sample_folder_config.csv.include_headers
        )
...
class TestNullAndEmptyHandling:
    ...
    def test_empty_email_config(self, temp_database):
        ...
        config = FolderConfiguration(folder_name="/test/path", email=None)
    ...
    def test_disabled_upc_override(self, temp_database):
        ...
        config = FolderConfiguration(
            ...
            upc_override=UPCOverrideConfiguration(enabled=False),  # Disabled
        )
...
class TestNestedConfigSerialization:
    ...
    def test_upc_config_serialization(self, temp_database):
        ...
        config = FolderConfiguration(
            folder_name="/test",
            upc_override=UPCOverrideConfiguration(
                enabled=True, level=3, category_filter="5,10,15", target_length=14
            ),
        )
...
class TestEDIFieldsMapping:
    ...
    def test_all_edi_fields_mapping(self, temp_database):
        ...
        config = FolderConfiguration(folder_name="/test/edi", edi=edi_config)
...
class TestCSVFieldsMapping:
    ...
    def test_all_csv_fields_mapping(self, temp_database):
        ...
        config = FolderConfiguration(folder_name="/test/csv", csv=csv_config)
