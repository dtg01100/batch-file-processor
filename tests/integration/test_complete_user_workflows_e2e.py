...
class TestAddFolderConfigureProcessWorkflow:
    ...
    def test_workflow_with_edi_validation(self, complete_workspace, sample_folder_config):
    ...
    def test_workflow_with_duplicate_detection(self, complete_workspace, sample_folder_config):
...
class TestErrorRecoveryWorkflow:
    ...
    def test_retry_after_backend_failure(self, complete_workspace, sample_folder_config):
    ...
    def test_continue_after_file_error(self, complete_workspace, sample_folder_config):
        ...
        config = DispatchConfig(
            backends={'copy': MockBackend(should_succeed=True)},
            settings={}
        )
        orchestrator = DispatchOrchestrator(config)
...
class TestMultiStepWorkflow:
    ...
    def test_process_multiple_folders_sequentially(self, complete_workspace):
        ...
        (input2 / "file1.edi").write_text("""A00000220240102002TESTVENDOR         Test Vendor Inc                 00002
...
""")
    ...
    def test_process_with_multiple_backends(self, complete_workspace, sample_folder_config):
...
class TestDatabaseIntegrationWorkflow:
    ...
    def test_processed_files_tracking(self, complete_workspace, sample_folder_config, temp_database):
        ...
        result = orchestrator.process_folder(folder_config, run_log, temp_database.processed_files)
        
        # Verify files tracked in database
        processed_count = temp_database.processed_files.count()
        assert processed_count >= 5

    def test_resend_flag_workflow(self, complete_workspace, sample_folder_config, temp_database):
        ...
        for file_record in list(files)[:2]:
            temp_database.processed_files.update(
                file_record['id'],
                {'resend_flag': 1}
            )
...
class TestEdgeCaseWorkflows:
    ...
    def test_large_file_processing(self, complete_workspace, sample_folder_config):
        ...
        large_content = "A00000120240101001TESTVENDOR         Test Vendor Inc                 00001\n" * 20000
    ...
    def test_special_characters_in_path(self, sample_folder_config):
        ...
        with tempfile.TemporaryDirectory(suffix="_test-folder") as tmpdir:
            ...
            # Create test file
            (special_input / "test.edi").write_text("""A00000120240101001TESTVENDOR         Test Vendor Inc                 00001
