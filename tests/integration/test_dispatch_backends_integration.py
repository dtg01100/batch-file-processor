...
class TestDispatchConvertBackendIntegration:
    ...
    def test_dispatch_uses_convert_backend_correctly(
        ...
        def mock_edi_convert(
            edi_process, output_filename, settings_dict_arg, parameters_dict, upc_lookup
        ):
        ...
        with patch.dict(
            "sys.modules",
            {"convert_to_fintech": MagicMock(edi_convert=mock_edi_convert)},
        ):
    ...
    def test_dispatch_sends_to_email_backend(
        ...
        with patch("email_backend.do", side_effect=mock_email_do):
            ...
            result = orchestrator.process_file(
                sample_edi_file, folder_config_with_email
            )
            ...
            assert (
                call["process_parameters"]["email_subject_line"] == "File: %filename%"
            )
...
class TestDispatchFullPipeline:
    ...
    def test_dispatch_full_pipeline(
        ...
        with (
            ...
            # Process the file
            result = orchestrator.process_file(
                sample_edi_file, folder_config_with_all_backends
            )
    ...
    def test_dispatch_full_pipeline_with_validation(
        ...
        with patch("copy_backend.do", side_effect=mock_copy_do):
            ...
            # Process the file
            result = orchestrator.process_file(
                sample_edi_file, folder_config_with_validation
            )
...
class TestSendManagerIntegration:
    ...
    def test_send_manager_with_injected_backend(self, sample_edi_file, settings_dict):
        ...
        results = manager.send_all({"test"}, sample_edi_file, params, settings_dict)
    ...
    def test_send_manager_send_all_success(self, sample_edi_file, settings_dict):
        ...
        with (
            ...
            results = manager.send_all(
                {"ftp", "email", "copy"}, sample_edi_file, params, settings_dict
            )
