...
class TestSequentialMultiFolderProcessing:
    ...
    def test_process_folders_with_different_configs(self, multiple_folders_workspace):
        ...
        for i, (folder, cfg) in enumerate(zip(multiple_folders_workspace['folders'][:3], configs)):
...
class TestResourceContention:
    ...
    def test_concurrent_folder_access(self, multiple_folders_workspace):
        ...
        for folder in multiple_folders_workspace['folders']:
            ...
            folder_config['copy_to_directory'] = str(shared_output / folder['config']['alias'].replace(' ', '_'))
    ...
    def test_database_locking_during_multi_folder(self, multiple_folders_workspace, temp_database):
        ...
        for folder_config in multiple_folders_workspace['folders']:
            ...
            folder_cfg['id'] = folder_config['folders_id'] if 'folders_id' in folder_config else None
            result = orchestrator.process_folder(folder_cfg, run_log, temp_database.processed_files)
            assert result.success is True
...
class TestLargeScaleMultiFolder:
    ...
    def test_process_20_folders(self, tmp_path):
        ...
        for i in range(20):
            ...
            for j in range(2):
                (input_dir / f"file_{j}.edi").write_text(f"""A00000{i}20240101001TESTVENDOR         Test Vendor Inc                 0000{i}
