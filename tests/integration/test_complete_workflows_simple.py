...
def test_environment():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        db = DatabaseObj(
            ...
            database_version="42",  # Use latest version
...
class TestCompleteFolderLifecycle:
    ...
    def test_create_configure_process_delete_workflow(self, test_environment, sample_edi_content, pipeline_steps):
        ...
        assert result.files_processed >= 0, f"Should process files, got {result.files_processed}"
        ...
        assert len(output_files) > 0, f"Should create output file. Processed: {result.files_processed}, Failed: {result.files_failed}"
...
class TestErrorRecoveryWorkflow:
    ...
    def test_processing_error_continues_with_other_folders(self, test_environment, sample_edi_content):
...
"""
        ...
        assert output_count == num_folders, \
