...
class TestResendFlow:
    ...
    def test_resend_with_duplicate_detection(
        self, temp_workspace, folder_config, mock_backend
    ):
    ...
    def test_resend_multiple_files(self, temp_workspace, folder_config, mock_backend):
        ...
        marked_records = list(
            processed_table.find(folder_id=folder_config["id"], resend_flag=True)
        )
...
class TestEDITweakingFlow:
    ...
    def test_date_offset_tweak(
        self, temp_workspace, complete_parameters_dict, complete_settings_dict
    ):
...
class TestFolderConfigurationFlow:
    ...
    def test_update_folder_config(self, temp_workspace):
        ...
        folders_table.update(
            {"id": result, "process_backend_copy": True, "alias": "UpdatedTest"}, ["id"]
        )
        ...
        assert (
            ...
            or config["process_backend_copy"] == 1
        )
