...
def workspace(tmp_path):
    ...
    output_dir_1 = workspace_dir / "output_1"
    output_dir_2 = workspace_dir / "output_2"
    processed_dir = workspace_dir / "processed"
    logs_dir = workspace_dir / "logs"
    errors_dir = workspace_dir / "errors"
    
    for d in [input_dir, output_dir_1, output_dir_2, processed_dir, logs_dir, errors_dir]:
        d.mkdir()
    ...
    yield {
        "dir": workspace_dir,
        "input": input_dir,
        "output_1": output_dir_1,
        "output_2": output_dir_2,
...
def test_full_real_world_workflow(workspace):
    ...
    assert result.files_processed == 1, f"Expected 1 file processed, got {result.files_processed}. Errors: {result.errors}"
    ...
    assert processed is not None, "File should have been recorded in processed_files table"
    ...
    assert result_skip.files_processed == 0, "File should have been skipped as duplicate"
    ...
    assert result_resend.files_processed == 1, "File should have been re-processed due to resend_flag"
    ...
    assert processed_after["resend_flag"] == 0, "resend_flag should be cleared after successful re-processing"
...
def test_tweaking_scenario(workspace):
    ...
    a_rec = "AVENDOR00000000010101260000001000\n"
    b_rec = f"B{upc_11}{description}{item}{cost}{combo}{multiplier}{qty}{retail}         \n"
    c_rec = "C001CHARGER DESCRIPTION      000000000\n"
    ...
    assert "B012345678905" in output_content, f"UPC should have check digit appended. Content: {output_content}"
...
def test_csv_conversion_with_options(workspace):
    ...
    if csv_files:
        ...
        # Should have headers
        assert "UPC" in csv_content or "VENDOR" in csv_content, "CSV should contain headers"
...
def test_all_conversion_formats(workspace):
    ...
    for fmt in SUPPORTED_FORMATS:
        ...
        assert result.files_processed == 1, f"Format {fmt} failed. Errors: {result.errors}"
...
def test_simplified_csv_with_options(workspace):
    ...
    if csv_files:
        ...
        if apple_pos > 0 and zebra_pos > 0:
            assert apple_pos < zebra_pos, "Items should be sorted alphabetically by description"
...
def test_email_backend(workspace):
    ...
    with mock.patch("backend.smtp_client.RealSMTPClient") as MockSMTP:
        ...
        assert mock_smtp_instance.send_message.called, "Email send_message should have been called"
...
def test_csv_option_individual(workspace, option, value, expected_behavior):
    ...
    if expected_behavior == "headers_present" and output_files[0].suffix == ".csv":
        ...
        assert "upc_number" in lines[0] or "UPC" in lines[0], "Headers should be present"
    elif expected_behavior == "no_headers" and output_files[0].suffix == ".csv":
        assert not ("upc_number" in lines[0] or "UPC" in lines[0]), "Headers should not be present"
    elif expected_behavior == "a_records_present" and output_files[0].suffix == ".csv":
        # For CSV, A records would be included as rows
        assert any("VENDOR" in line for line in lines), "A records should be present in CSV"
        ...
        assert any("CHARGER" in line or "CHARGE" in line for line in lines), "C records should be present in CSV"
    ...
    # For other cases, just verify the file was created and has content
        ...
        assert len(output_content) > 0, f"Output file should have content for {option}={value}"
...
def test_simplified_csv_option_individual(workspace, option, value):
    ...
    assert len(output_files) >= 1, f"Expected CSV output for simplified {option}={value}"
...
@pytest.mark.parametrize("tweak_option,value", [
    ("calculate_upc_check_digit", True),
    ("pad_a_records", True),
...
@pytest.mark.parametrize("combo_name,config_overrides,description", [
...
@pytest.mark.parametrize("format_name,options", [
...
def test_all_formats_with_options(workspace, format_name, options):
    ...
    c_rec = "C001CHARGER DESCRIPTION      000000000\n"
    test_file_content = a_rec + b_rec + c_rec
    test_file_path = workspace["input"] / f"test_{format_name}_{hash(frozenset(options.items()))}.edi"
    test_file_path.write_text(test_file_content)
    ...
    assert result.files_processed == 1, f"{format_name} with {options} - Errors: {result.errors}"
...
@pytest.mark.parametrize("backend_type,backend_config", [
    ...
    ("ftp", {"process_backend_ftp": 1, "ftp_server": "ftp.example.com", "ftp_port": 21, "ftp_username": "user", "ftp_password": "pass", "ftp_folder": "/uploads/"}),
    ("email", {"process_backend_email": 1, "email_to": "test@example.com", "email_subject_line": "Test"}),
])
