...
class TestProcessingWorkflow:
    ...
    def test_process_single_folder_workflow(self, initialized_app, temp_workspace):
        ...
        # Add folder
        app._folder_manager.add_folder(input_folder)
        app._database.folders_table.find_one(folder_name=input_folder)
...
class TestMaintenanceWorkflow:
    ...
    def test_imported_db_dependent_interactions_and_run_with_odbc_mocked(
        ...
        monkeypatch.setattr(
            "backend.copy_backend.do",
            _mock_copy_backend,
        )
        monkeypatch.setattr("backend.ftp_backend.do", _mock_ftp_backend)
        monkeypatch.setattr("backend.email_backend.do", _mock_email_backend)
        ...
        def _wrapped_process_folder(self, folder, run_log, processed_files, **kwargs):
            folder_id = folder.get("id")
            if folder_id:
                processed_folder_ids.add(folder_id)
            return original_process_folder(
                self, folder, run_log, processed_files, **kwargs
            )

        def _track_process_folder_calls():
            """Factory to create a wrapper that tracks which folders are processed."""

            def _inner_wrapper(self, folder, run_log, processed_files, **kwargs):
                folder_id = folder.get("id")
                if folder_id:
                    processed_folder_ids.add(folder_id)
                return original_process_folder(
        ...
        # Verify that the processing run iterated over the active folders.
        # On Linux the watched directories (Windows I: paths) don't exist so the
        # run produces 0 files per folder, but process_folder is still called for
        # each active folder via the _process_directories fallback path.
        assert len(processed_folder_ids) > 0, (
            f"Expected at least some folders to be processed, got none. "
            f"Active folder count: {len(active_folder_ids)}"
        )
...
class TestPersistenceWorkflow:
    ...
    def test_settings_persist_workflow(self, initialized_app):
        ...
        app._database.get_settings_or_default()
