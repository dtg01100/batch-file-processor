...
def mock_backends(temp_workspace):
    ...
    class MockCopyBackend:
        ...
        def send(self, params: dict, settings: dict, filename: str) -> bool:
            ...
            dest_dir = Path(
                params.get("copy_to_directory", temp_workspace["output_folder"])
            )
...
class TestEndToEndBatchProcessing:
    ...
    def test_full_batch_processing_flow(
        self, temp_workspace, folder_config, mock_backends
    ):
        ...
        """
        # Create orchestrator
        config = DispatchConfig(backends=mock_backends, settings={"test": "value"})
    ...
    def test_backend_failure_handling(self, temp_workspace, folder_config):
        ...
        config = DispatchConfig(
            backends={"copy": FailingBackend()}, settings={"test": "value"}
        )
        orchestrator = DispatchOrchestrator(config)
        run_log = MagicMock()
    ...
    def test_backend_exception_handling(self, temp_workspace, folder_config):
        ...
        config = DispatchConfig(
            backends={"copy": ExceptionBackend()}, settings={"test": "value"}
        )
        orchestrator = DispatchOrchestrator(config)
        run_log = MagicMock()
    ...
    def test_empty_folder_processing(
        self, temp_workspace, folder_config, mock_backends
    ):
    ...
    def test_missing_folder_handling(
        self, temp_workspace, folder_config, mock_backends
    ):
    ...
    def test_mixed_success_and_failure(self, temp_workspace, folder_config):
        ...
        config = DispatchConfig(
            backends={"copy": SelectiveBackend()}, settings={"test": "value"}
        )
        orchestrator = DispatchOrchestrator(config)
        run_log = MagicMock()
    ...
    def test_file_filters_edi_files_only(
        self, temp_workspace, folder_config, mock_backends
    ):
...
class TestBatchProcessingPerformance:
    ...
    def test_processes_multiple_files_efficiently(
        self, temp_workspace, folder_config, mock_backends, sample_edi_content
    ):
