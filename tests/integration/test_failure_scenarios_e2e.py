...
def temp_workspace(sample_edi_content):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        (input_folder / "invoice_002.edi").write_text(
            sample_edi_content.replace("00001", "00002")
        )
        (input_folder / "invoice_003.edi").write_text(
            sample_edi_content.replace("00001", "00003")
        )
...
class TestBackendFailureScenarios:
    ...
    def test_multiple_backend_failures_with_partial_recovery(
        self, temp_workspace, folder_config
    ):
        ...
        config = DispatchConfig(
            backends={
                "copy": IntermittentFailureBackend(),
                "email": AlwaysSuccessBackend(),
            },
            settings={"test": "value"},
        )
        ...
        ), "Overall should be failure due to partial failures"

    def test_all_backends_fail_with_error_logging(
        self, temp_workspace, folder_config, caplog
    ):
        ...
        class LoggingFailureBackend:
            def send(self, params: dict, settings: dict, filename: str) -> None:
                failure_details.append(
                    {"filename": filename, "params": params, "settings": settings}
                )
                raise Exception("Logging backend failure")

        config = DispatchConfig(
            backends={"copy": LoggingFailureBackend()}, settings={"test": "value"}
        )
        orchestrator = DispatchOrchestrator(config)
        run_log = MagicMock()
        ...
        assert (
            "Backend 'copy' failed to send: Logging backend failure" in caplog.text
        ), "Error should be logged via Python logging"

    def test_exception_in_one_backend_does_not_stop_others(
        self, temp_workspace, folder_config
    ):
        ...
        config = DispatchConfig(
            backends={
                ...
                "success_backend": SuccessBackend(),
            },
            settings={"test": "value"},
        )
        orchestrator = DispatchOrchestrator(config)
    ...
    def test_cascading_backend_failures(self, temp_workspace, folder_config):
        ...
        config = DispatchConfig(
            backends={
                "ftp": CascadingFailureBackend(fail_after_call=2),
...
class TestFileProcessingFailureScenarios:
    ...
    def test_corrupted_file_handling(self, temp_workspace, folder_config):
        ...
        config = DispatchConfig(
            backends={"copy": SuccessBackend()}, settings={"test": "value"}
        )
        orchestrator = DispatchOrchestrator(config)
        run_log = MagicMock()
        ...
        assert (
            len(success_calls) == 4
        ), "Without validation enabled, all 4 files should be sent"
        assert (
            result.files_processed + result.files_failed == 4
        ), "All 4 files should be attempted"
    ...
    def test_file_access_permission_failures(self, temp_workspace, folder_config):
        ...
        try:
            ...
            assert result is not None
            assert (
                result.files_processed + result.files_failed == 3
            ), "All 3 files should be attempted"
        finally:
    ...
    def test_output_folder_unavailable(self, temp_workspace, folder_config):
        ...
        config = DispatchConfig(
            backends={"copy": FailingCopyBackend()}, settings={"test": "value"}
        )
        orchestrator = DispatchOrchestrator(config)
        run_log = MagicMock()
...
class TestErrorRecoveryWorkflows:
        ...
        class RetryAwareBackend:
            def send(self, params: dict, settings: dict, filename: str) -> None:
                ...
                if attempt_count[filename] < 2:
                    raise Exception(
                        f"Backend failed on attempt {attempt_count[filename]} for {filename}"
                    )

        config = DispatchConfig(
            backends={"copy": RetryAwareBackend()}, settings={"test": "value"}
        )
        orchestrator = DispatchOrchestrator(config)
        run_log = MagicMock()
    ...
    def test_error_isolation(self, temp_workspace, folder_config):
        ...
        config = DispatchConfig(
            backends={"copy": TrackingBackend()}, settings={"test": "value"}
        )
        orchestrator = DispatchOrchestrator(config)
        run_log = MagicMock()
        ...
        assert (
            result.success is False
        ), "Overall success should be False due to failures"


class TestDatabaseConnectionFailures:
    """Test scenarios where database operations fail."""

    def test_database_unavailable_during_processing(
        self, temp_workspace, folder_config
    ):
