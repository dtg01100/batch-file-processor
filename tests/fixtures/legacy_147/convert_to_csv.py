        ...
        with open(
            ...
            for line_num, line in enumerate(
                ...
                if input_edi_dict is not None:
                    ...
                    if (
                        input_edi_dict["record_type"] == "A"
                        and conv_inc_arec != "False"
                    ):
                        # write "A" line
                        if pad_arec == "True":
                        ...
                        csv_file.writerow(
                            [
                                input_edi_dict["record_type"],
                                cust_vendor,
                                input_edi_dict["invoice_number"],
                                input_edi_dict["invoice_date"],
                    ...
                    if input_edi_dict["record_type"] == "B":
                        if retail_uom:
                            ...
                            if edi_line_pass:
                                try:
                                    each_upc_string = upc_lut[item_number][1][
                                        :11
                                    ].ljust(11)
                                except KeyError:
                                    each_upc_string = "           "
                        ...
                        if override_upc:
                            try:
                                if (
                                    int(input_edi_dict["vendor_item"].strip())
                                    in upc_lut.keys()
                                ):
                                    do_updateupc = False
                                    if override_upc_category_filter == "ALL":
                                        ...
                                        if upc_lut[
                                            int(input_edi_dict["vendor_item"].strip())
                                        ][0] in override_upc_category_filter.split(","):
                                            do_updateupc = True
                                    if do_updateupc:
                                        input_edi_dict["upc_number"] = upc_lut[
                                            int(input_edi_dict["vendor_item"].strip())
                                        ][override_upc_level]
                                else:
                        ...
                        upc_in_csv = (
                            "\t" + upc_string
                            if conv_calc_upc != "False" and blank_upc is False
                            else input_edi_dict["upc_number"]
                        )

                        quantity_shipped_in_csv = (
                            input_edi_dict["qty_of_units"].lstrip("0")
                            if not input_edi_dict["qty_of_units"].lstrip("0") == ""
                            else input_edi_dict["qty_of_units"]
                        )

                        cost_in_csv = convert_to_price(input_edi_dict["unit_cost"])

                        suggested_retail_in_csv = convert_to_price(
                        ...
                        description_in_csv = (
                            ...
                            .replace("&", "AND")
                            .rstrip(" ")
                            if filter_ampersand != "False"
                            else input_edi_dict["description"].rstrip(" ")
                        )

                        case_pack_in_csv = (
                            input_edi_dict["unit_multiplier"].lstrip("0")
                            if not input_edi_dict["unit_multiplier"].lstrip("0") == ""
                            else input_edi_dict["unit_multiplier"]
                        )

                        item_number_in_csv = (
                            input_edi_dict["vendor_item"].lstrip("0")
                            if not input_edi_dict["vendor_item"].lstrip("0") == ""
                            else input_edi_dict["vendor_item"]
                        )
