...
def process(
    ...
    def validate_file(input_file, file_name):
        ...
        if minor_edi_errors or (
            edi_validator_error_status and reporting["report_edi_errors"]
        ):
    ...
    for parameters_dict in parameters_dict_list:
        ...
        if os.path.isdir(parameters_dict["folder_name"]) is True:
            ...
            run_log.write(b"Checking for new files\r\n")
            print("Checking for new files")

            file_count = 0
            folder_errors = False
            if len(files) == 0:  # if there are no files in directory, record in log
                run_log.write(b"No files in directory\r\n\r\n")
                print("No files in directory")
            if len(filtered_files) == 0 and len(files) > 0:
                run_log.write(b"No new files in directory\r\n\r\n")
                    ...
                    if parameters_dict["split_edi"] and valid_edi_file:
                        ...
                        try:
                            split_edi_list = utils.do_split_edi(
                                input_filename, file_scratch_folder, parameters_dict
                            )
    ...
    run_summary_string = f"{processed_counter!s} processed, {error_counter!s} errors{edi_validator_error_report_string}"
    run_log.write(("\r\n\r\n" + run_summary_string + "\r\n\r\n").encode())
    if error_counter > 0:
