...
MUTATIONS: list[tuple[str, re.Pattern[str], Callable[[re.Match[str]], str]]] = [
    ...
    # Membership swaps: `x in y` <-> `x not in y`. Use word boundaries
    # to avoid matching `int`, `print`, `join`, etc. Order matters:
    # `not_in_to_in` must run before `in_to_not_in` so the `not in`
    # rule is consumed first (otherwise `not in` would match `in` and
    # produce `not not in`).
    (
        "not_in_to_in",
...
DEFAULT_PAIRS: list[tuple[str, str]] = [
    ...
    (
        ...
        "tests/unit/core/edi/test_c_rec_generator_property.py",
    ),
    (
    ...
    (
        ...
        "tests/unit/dispatch/test_converters_registry.py",
    ),  # 3/7 killed
    (
    ...
    (
        "core/edi/inv_fetcher.py",
        "tests/unit/core/edi/test_inv_fetcher.py",
    ),  # 7/9 (was 5/9; L122 dict boundary, L123 len check killed by test_fetch_po_handles_single_field_dict; L171 true_to_false is KNOWN_EQUIVALENT because Python's logging auto-populates exc_info inside except blocks)
    (
        "adapters/db2ssh/connection.py",
        "tests/unit/adapters/db2ssh/test_db2ssh_connection.py",
    ),  # 3/7 killed
    (
        "core/edi/edi_parser.py",
        "tests/unit/core/edi/test_edi_parser.py",
    ),  # 1/10 killed (note: distinct pair from the property test in DEFAULT_PAIRS above)
    # Batch 2 (also 2026-07-09):
    (
        "core/database/query_runner.py",
        "tests/unit/core/database/test_query_runner.py",
    ),  # 6/10 killed
    (
        "backend/copy_backend.py",
        "tests/unit/backend/test_copy_backend.py",
    ),  # 4/8 killed
    (
    ...
    (
        ...
        "tests/unit/dispatch/services/test_database_connector.py",
    ),  # 5/5 killed (100% clean)
    (
    ...
    (
        "core/edi/c_rec_generator.py",
        "tests/unit/core/edi/test_c_rec_generator.py",
    ),  # 4/9 killed
    (
        "dispatch/feature_flags.py",
    ...
    (
        "dispatch/processed_files_tracker.py",
    ...
    ),  # 4/8 (was 0/8; 4 docstring mutations KNOWN_EQUIVALENT, 4 real mutations killed by TestNormalizeValidationOutput)
    (
        "dispatch/pipeline/temp_dir_utils.py",
        "tests/unit/dispatch_tests/test_pipeline_temp_dir_utils.py",
    ),  # 2/4 killed
    (
        "dispatch/edi_validator.py",
        "tests/unit/dispatch_tests/test_edi_validator.py",
    ),  # 5/10 killed
    (
        "dispatch/preflight_validator.py",
        "tests/unit/dispatch_tests/test_preflight_validator.py",
    ),  # 3/6 killed
    (
        "dispatch/log_sender.py",
        "tests/unit/dispatch_tests/test_log_sender.py",
    ),  # 4/8 killed
    (
        "dispatch/error_handler.py",
        "tests/unit/dispatch_tests/test_error_handler.py",
    ),  # 4/7 (was 1/7; L91 or_to_and, L154 negate, L193 false_to_true, L224 return_none killed by test_init_preserves_truthy_errors_folder, test_record_error_to_database, test_default_threaded_is_false, test_record_error_to_logs_threaded)
    (
    ...
    (
        "dispatch/converters/convert_base.py",
        "tests/unit/test_convert_base.py",
    ),  # 4/9 (was 3/9; L128 false_to_true on output_file repr=False killed by test_output_file_excluded_from_repr)
    (
        "dispatch/interfaces.py",
    ...
    (
        "core/edi/edi_splitting_utils.py",
        "tests/unit/test_category_filtering.py",
    ),  # was mispaired as edi_splitter.py (0/7 was a DEFAULT_PAIRS bug, not a test gap)
    (
    ...
    (
        "dispatch/converters/convert_to_csv.py",
        "tests/unit/test_convert_to_csv.py",
    ),  # was mispaired as convert_to_simplified_csv (0/8 was a DEFAULT_PAIRS bug, not a test gap)
    (
        "dispatch/converters/convert_to_yellowdog_csv.py",
        "tests/unit/test_convert_to_yellowdog_csv.py",
    ),  # was mispaired as core/edi/inv_fetcher (0/9 was a DEFAULT_PAIRS bug, not a test gap)
    (
        "dispatch/converters/convert_to_fintech.py",
        "tests/unit/test_convert_to_fintech.py",
    ),  # was mispaired as core/edi/inv_fetcher (0/9 was a DEFAULT_PAIRS bug, not a test gap)
    (
        "dispatch/converters/convert_to_scansheet_type_a.py",
        "tests/unit/test_convert_to_scansheet_type_a.py",
    ),  # 1/9 (commit d14b686d5)
    (
        "dispatch/converters/convert_to_estore_einvoice.py",
        "tests/unit/test_convert_to_estore_einvoice.py",
    ),  # 6/8 killed (strong test)
    (
        "dispatch/converters/convert_to_estore_einvoice_generic.py",
        "tests/unit/test_convert_to_estore_einvoice_generic.py",
    ),  # 6/8 killed (strong test)
    (
        "dispatch/converters/registry.py",
        "tests/unit/dispatch_tests/test_legacy_147_smoke.py",
    ),  # 1/7 killed (companion to test_converters_registry.py)
]
...
KNOWN_EQUIVALENT: list[tuple[str, str, int, str]] = [
    ...
    ("dispatch/file_utils.py", "false_to_true", 128, "same docstring line — prose"),
    (
        "dispatch/file_utils.py",
        "and_to_or",
        31,
    ...
    ("core/utils/bool_utils.py", "false_to_true", 8, "same docstring line — prose"),
    (
        "core/utils/bool_utils.py",
        "and_to_or",
        74,
    ...
    *,
    ...
    for name, pattern, replace in MUTATIONS:
        ...
        if code == 0:
            report.survived += 1
            report.outcomes.append(
                MutationOutcome(name=name, line=line, killed=False, snippet=snippet)
            )
        else:
            report.killed += 1
            report.outcomes.append(
...
def render_report(report: ModuleReport) -> str:
    ...
    lines.append(
        f"Killed:    {report.killed}/{report.total} " f"({report.kill_rate:.0%})"
    )
    ...
    if report.survived:
        ...
        for outcome in report.outcomes:
            if not outcome.killed:
                lines.append(f"  - line {outcome.line:>4}: {outcome.name}")
                if outcome.snippet:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.pair_list:
        for raw in args.pair_list:
            if ":" not in raw:
                print(
                    f"--pair-list entry must be MODULE:TEST, got: {raw}",
                    file=sys.stderr,
                )
                return 2
...
def _default_pairs_all_exist() -> tuple[list[str], list[str]]:
    """Sanity-check that every entry in ``DEFAULT_PAIRS`` resolves to a
    real module and a real test file under ``PROJECT_ROOT``.

    Returns (missing_modules, missing_tests). Either list non-empty means
    a pair is stale and will silently produce 0-mutation reports at
    runtime — better to surface the stale entry here.
    """
    ...
    return missing_modules, missing_tests


def test_default_pairs_resolve() -> None:
    """Every (module, test_file) in ``DEFAULT_PAIRS`` must resolve to a
    real file. Stale entries silently produce 0-mutation reports — this
    test surfaces the drift so a contributor fixing a deletion notices.
    """
    missing_modules, missing_tests = _default_pairs_all_exist()
    failures: list[str] = []
    if missing_modules:
        failures.append(
            "DEFAULT_PAIRS references missing module(s):\n  "
            + "\n  ".join(missing_modules)
        )
    if missing_tests:
        failures.append(
            "DEFAULT_PAIRS references missing test file(s):\n  "
            + "\n  ".join(missing_tests)
        )
    if failures:
        pytest.fail("\n".join(failures))
...
def test_mutation_runner_self_check() -> None:
    """Sanity check: the runner file itself parses and exposes the
    public surface (``DEFAULT_PAIRS``, ``KNOWN_EQUIVALENT``,
    ``run_meta_test``).

    Parity with ``test_hygiene_runner_self_check`` and
    ``test_assertion_runner_self_check``: this runner guards itself
    against self-inflicted breakage (e.g. an accidental rename of
    ``DEFAULT_PAIRS`` would silently turn the meta-test into a no-op).
    """
    self_path = Path(__file__).resolve()
    source = self_path.read_text(encoding="utf-8", errors="replace")
    try:
        ast.parse(source, filename=str(self_path))
    ...
    module_globals = {
        ...
        "run_meta_test": run_meta_test,
    }
    for name in ("DEFAULT_PAIRS", "KNOWN_EQUIVALENT", "run_meta_test"):
        if name not in module_globals or module_globals[name] is None:
            pytest.fail(f"mutation runner is missing required top-level name: {name}")
