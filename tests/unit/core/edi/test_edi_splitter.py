...
class TestSplitConfig:
    ...
    def test_split_config_all_values(self):
        ...
        config = SplitConfig(
            output_directory="/tmp/output", prepend_date=True, max_invoices=100
        )
...
class TestEDISplitter:
    ...
    def test_split_edi_basic(
        self, splitter, mock_filesystem, sample_edi_content, config
    ):
    ...
    def test_split_edi_creates_output_directory(
        self, splitter, mock_filesystem, sample_edi_content, config
    ):
