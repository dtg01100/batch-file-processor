...
class TestPrintRunLog:
    ...
    def test_log_formatting_wraps_long_lines(self, sample_log_content):
        ...
        long_line = "A" * 200
        wrapped = "\r\n".join(
            textwrap.wrap(long_line, width=75, replace_whitespace=False)
        )
    ...
    def test_empty_log_content(self):
        ...
        empty_content = ""
        wrapped = "\r\n".join(
            textwrap.wrap(empty_content, width=75, replace_whitespace=False)
        )
        assert wrapped == ""
    ...
    def test_single_character_log(self):
        ...
        single_char = "A"
        wrapped = "\r\n".join(
            textwrap.wrap(single_char, width=75, replace_whitespace=False)
        )
        assert wrapped == "A"
    ...
    def test_exact_width_line(self):
        ...
        exact_width = "A" * 75
        wrapped = "\r\n".join(
            textwrap.wrap(exact_width, width=75, replace_whitespace=False)
        )
    ...
    def test_multiline_log_preserves_structure(self):
        ...
        wrapped = "\r\n".join(
            textwrap.wrap(multiline, width=75, replace_whitespace=False)
        )
...
class TestPrintRunLogWindows:
    ...
    def test_printer_workflow_sequence(self, mock_win32print):
        ...
        mock_win32print.StartDocPrinter(
