...
class MockDatabase:
    ...
    def find(self, **kwargs) -> list[dict]:
        return [
            r for r in self.records if all(r.get(k) == v for k, v in kwargs.items())
        ]
...
class TestDispatchOrchestrator:
    ...
    def test_init_with_components(self):
        ...
        config = DispatchConfig(
            backends={"test": MockBackend()}, settings={"key": "value"}
        )
        orchestrator = DispatchOrchestrator(config)
    ...
    def test_process_folder_with_files(self, caplog):
        ...
        mock_fs.list_files = lambda path: (
            ["/data/input/file1.edi", "/data/input/file2.edi"]
            if path in mock_fs.dirs
            else []
        )
...
class TestOrchestratorIntegration:
    ...
    def test_multiple_folders(self):
        ...
        mock_fs.list_files = lambda path: [
            f for f in mock_fs.files if f.startswith(path)
        ]
        ...
        folders = [
            {
                "folder_name": "/data/input1",
                "alias": "Input1",
                "process_backend_copy": True,
            },
            {
                "folder_name": "/data/input2",
                "alias": "Input2",
                "process_backend_copy": True,
            },
        ]
