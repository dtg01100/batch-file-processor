...
def mock_file_system(edi_content_single_invoice):
    ...
    return MockFileSystem({"/test/input.edi": edi_content_single_invoice})
...
class TestSplitterResult:
    ...
    def test_custom_values(self):
        ...
        files = [
            ("/output/file1.inv", "A_", ".inv"),
            ("/output/file2.inv", "B_", ".inv"),
        ]
...
class TestMockSplitter:
    ...
    def test_initialization_with_custom_result(self):
        ...
        custom_result = SplitterResult(
            files=[("/output/file.inv", "A_", ".inv")], was_split=True
        )
    ...
    def test_call_tracking(self):
        ...
        splitter.split(
            "/input/test.edi", "/output", {"split_edi": True}, {"upc": "dict"}
        )
    ...
    def test_set_result_method(self):
        ...
        new_result = SplitterResult(
            files=[("/new/file.inv", "X_", ".inv")], was_split=True, was_filtered=True
        )
    ...
    def test_split_returns_configured_result(self):
        ...
        custom_result = SplitterResult(
            files=[("/output/test.inv", "A_", ".inv")], was_split=True
        )
        splitter = MockSplitter(result=custom_result)
...
class TestEDISplitterStep:
    ...
    def test_split_error_recording_to_error_handler(self):
        ...
        step = EDISplitterStep(splitter=mock_splitter, error_handler=mock_error_handler)
    ...
    def test_split_parameter_normalization_include_invoices_string(self):
        ...
        params = {"split_edi": True, "split_edi_include_invoices": "false"}
        result = step.split("/input/test.edi", "/output", params, {})
...
class TestIntegration:
    ...
    def test_full_split_flow_with_mock_file_system(self, edi_content_multiple_invoices):
        ...
        mock_fs = MockFileSystem({"/input/test.edi": edi_content_multiple_invoices})
        mock_error_handler = MockErrorHandler()

        step = EDISplitterStep(error_handler=mock_error_handler, file_system=mock_fs)
    ...
    def test_filtering_without_split_no_filter(self):
        ...
        params = {"split_edi": False, "split_edi_filter_categories": "ALL"}
        result = step.split("/input/test.edi", "/output", params, {})
...
class TestEdgeCases:
    ...
    def test_empty_files_in_result(self):
        ...
        mock_splitter = MockEDISplitter()
        mock_splitter.set_result(
            SplitResult(output_files=[], skipped_invoices=0, total_lines_written=0)
        )
        step = EDISplitterStep(splitter=mock_splitter)
    ...
    def test_large_number_of_output_files(self):
        ...
        output_files = [(f"/output/{i}_split.inv", f"{i}_", ".inv") for i in range(100)]

        mock_splitter = MockEDISplitter()
        mock_splitter.set_result(
            SplitResult(
                output_files=output_files, skipped_invoices=0, total_lines_written=300
            )
        )
        step = EDISplitterStep(splitter=mock_splitter)
