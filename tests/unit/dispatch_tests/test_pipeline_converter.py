...
from webapp.pipeline.pipeline.converter import (
    SUPPORTED_FORMATS,
    ConverterResult,
    DefaultModuleLoader,
    EDIConverterStep,
...
class MockFileSystem:
    ...
    def file_exists(self, path: str) -> bool:
        return (
            path in self.files or path in self.text_files or path in self.binary_files
        )
    ...
    def list_files(self, path: str) -> list[str]:
        all_files = (
            ...
            + list(self.text_files.keys())
            + list(self.binary_files.keys())
        )
...
class TestMockConverter:
    ...
    def test_initialization_with_custom_result(self):
        ...
        custom_result = ConverterResult(
            output_path="/output/custom.csv", format_used="csv", success=True, errors=[]
        )
    ...
    def test_set_result_method(self):
        ...
        new_result = ConverterResult(
            output_path="/new/output.csv", format_used="csv", success=True, errors=[]
        )
    ...
    def test_convert_returns_configured_result(self):
        ...
        custom_result = ConverterResult(
            output_path="/output/test.csv", format_used="csv", success=True, errors=[]
        )
        converter = MockConverter(result=custom_result)
...
class TestEDIConverterStep:
    ...
    def test_convert_with_process_edi_not_true(self):
        ...
        params = {"convert_to_format": "csv", "process_edi": "False"}
        result = step.convert("/input/test.edi", "/output", params, {}, {})
    ...
    def test_convert_with_unsupported_format(self):
        ...
        params = {"convert_to_format": "unsupported_format", "process_edi": "True"}
        result = step.convert("/input/test.edi", "/output", params, {}, {})
    ...
    def test_convert_with_supported_format_and_mock_module_loader(self, caplog):
        ...
        mock_loader = MockModuleLoader(
            {"webapp.pipeline.converters.convert_to_csv": mock_module}
        )

        step = EDIConverterStep(module_loader=mock_loader)

        params = {"convert_to_format": "csv", "process_edi": "True"}
        with caplog.at_level(logging.DEBUG, logger="webapp.pipeline.pipeline.converter"):
            result = step.convert("/input/test.edi", "/output", params, {}, {})
        ...
        assert result.success is True
        assert result.errors == []
        assert mock_loader.last_module_name == "webapp.pipeline.converters.convert_to_csv"
    ...
    def test_convert_handles_import_error(self, caplog):
        ...
        step = EDIConverterStep(
            module_loader=mock_loader, error_handler=mock_error_handler
        )
    ...
    def test_format_normalization_with_spaces(self):
        ...
        mock_module = create_mock_conversion_module("/output/converted.csv")
        mock_loader = MockModuleLoader(
            {"webapp.pipeline.converters.convert_to_estore_einvoice": mock_module}
        )

        step = EDIConverterStep(module_loader=mock_loader)

        params = {"convert_to_format": "estore einvoice", "process_edi": "True"}
        result = step.convert("/input/test.edi", "/output", params, {}, {})

        assert result.success is True
        assert (
            mock_loader.last_module_name
            == "webapp.pipeline.converters.convert_to_estore_einvoice"
        )
...
class TestIntegration:
    ...
    def test_full_conversion_flow_with_mock_module_loader(self):
        ...
        mock_module.edi_convert.assert_called_once_with(
            "/input/test.edi", "/output/test.edi", settings, params, upc_dict
        )
    ...
    def test_conversion_with_all_supported_formats(self):
        ...
        for format_name in ["csv", "fintech", "scannerware"]:
            mock_module = create_mock_conversion_module(
                f"/output/converted.{format_name}"
            )
            mock_loader = MockModuleLoader(
                {f"webapp.pipeline.converters.convert_to_{format_name}": mock_module}
            )

            step = EDIConverterStep(module_loader=mock_loader)

            params = {"convert_to_format": format_name, "process_edi": "True"}
            result = step.convert("/input/test.edi", "/output", params, {}, {})
...
class TestEdgeCases:
    ...
    def test_format_with_mixed_spaces_and_hyphens(self):
        ...
        mock_module = create_mock_conversion_module("/output/converted.csv")
        mock_loader = MockModuleLoader(
            {"webapp.pipeline.converters.convert_to_estore_einvoice_generic": mock_module}
        )

        step = EDIConverterStep(module_loader=mock_loader)

        params = {"convert_to_format": "estore einvoice-generic", "process_edi": "True"}
        result = step.convert("/input/test.edi", "/output", params, {}, {})

        assert result.success is True
        assert (
            mock_loader.last_module_name
            == "webapp.pipeline.converters.convert_to_estore_einvoice_generic"
        )
    ...
    def test_large_number_of_errors_recorded(self):
        ...
        params = {"convert_to_format": "invalid_format_1", "process_edi": "True"}
        result = step.convert("/input/test.edi", "/output", params, {}, {})
    ...
    def test_convert_to_format_is_source_of_truth(self):
        ...
        mock_loader = MockModuleLoader(
            {
                "webapp.pipeline.converters.convert_to_tweaks": tweak_mock_module,
                "webapp.pipeline.converters.convert_to_estore_einvoice": estore_mock_module,
            }
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("csv", "True", False, "webapp.pipeline.converters.convert_to_csv"),
            ("CSV", True, False, "webapp.pipeline.converters.convert_to_csv"),
            (
                "eStore eInvoice",
                "1",
            ...
            (
                ...
                "yes",
                False,
                "webapp.pipeline.converters.convert_to_yellowdog_csv",
            ),
            (
                "scansheet-type-a",
                "on",
                False,
                "webapp.pipeline.converters.convert_to_scansheet_type_a",
            ),
            ("tweaks", "True", True, "webapp.pipeline.converters.convert_to_tweaks"),
        ],
    )
    ...
    def test_tweaks_format_routes_to_tweaks_converter(self):
        ...
        assert result.success is True
        assert mock_loader.last_module_name == "webapp.pipeline.converters.convert_to_tweaks"
