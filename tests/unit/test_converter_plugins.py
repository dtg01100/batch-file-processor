...
from convert_to_estore_einvoice import EstoreEinvoiceConverter, edi_convert as estore_edi_convert
from convert_to_estore_einvoice_generic import EstoreEinvoiceGenericConverter, edi_convert as estore_generic_edi_convert
from convert_to_fintech import FintechConverter, edi_convert as fintech_edi_convert
from convert_to_jolley_custom import JolleyCustomConverter, edi_convert as jolley_edi_convert, CustomerLookupError
from convert_to_scannerware import ScannerWareConverter, edi_convert as scannerware_edi_convert
from convert_to_scansheet_type_a import ScansheetTypeAConverter, edi_convert as scansheet_edi_convert
from convert_to_simplified_csv import SimplifiedCsvConverter, edi_convert as simplified_edi_convert
from convert_to_stewarts_custom import StewartsCustomConverter, edi_convert as stewarts_edi_convert, CustomerLookupError as StewartsCustomerLookupError
from convert_to_yellowdog_csv import YellowdogConverter, edi_convert as yellowdog_edi_convert
...
def mock_query_runner():
    ...
    def mock_query_result(query_string):
        ...
        if "ohhst" in query_lower and ("bte4cd" in query_lower or "bthinb" in query_lower):
            return [("PO12345", "Test Customer")]
        elif "odhst" in query_lower and "buhhnb" in query_lower and "buhunb" in query_lower:
            return [(1, "EA"), (2, "CS"), (3, "BX")]
        elif "dsanrep" in query_lower and "anbacd" in query_lower:
            return [("Each",)]
        elif "odhst" in query_lower and "bubacd" in query_lower and "bus3qt" in query_lower:
...
class TestCsvConverter:
    ...
    def test_converter_initialization(self, sample_edi_file, temp_dir, settings_dict, upc_lookup):
    ...
    def test_happy_path_conversion(self, sample_edi_file, temp_dir, settings_dict, upc_lookup):
    ...
    def test_conversion_without_headers(self, sample_edi_file, temp_dir, settings_dict, upc_lookup):
    ...
    def test_include_a_records_option(self, sample_edi_file, temp_dir, settings_dict, upc_lookup):
    ...
    def test_include_c_records_option(self, sample_edi_file, temp_dir, settings_dict, upc_lookup):
    ...
    def test_filter_ampersand_option(self, sample_edi_file, temp_dir, settings_dict, upc_lookup):
    ...
    def test_retail_uom_option(self, sample_edi_file, temp_dir, settings_dict, upc_lookup):
    ...
    def test_empty_file_handling(self, empty_edi_file, temp_dir, settings_dict, upc_lookup):
    ...
    def test_complex_invoice_conversion(self, complex_edi_file, temp_dir, settings_dict, upc_lookup):
...
class TestEstoreEinvoiceGenericConverter:
    ...
    def test_happy_path_conversion(self, mock_query, sample_edi_file, temp_dir, settings_dict, upc_lookup, mock_query_runner):
...
class TestFintechConverter:
    ...
    def test_happy_path_conversion(self, mock_invfetcher_class, sample_edi_file, temp_dir, settings_dict, upc_lookup):
...
class TestJolleyCustomConverter:
    ...
    @patch("convert_to_jolley_custom.query_runner")
    def test_customer_lookup_error(self, mock_query, sample_edi_file, temp_dir, settings_dict, upc_lookup):
        """Test handling when customer data is not found."""
        mock_runner = MagicMock()
...
class TestScannerWareConverter:
    ...
    def test_pad_a_records_option(self, sample_edi_file, temp_dir, settings_dict, upc_lookup):
    ...
    def test_append_a_records_option(self, sample_edi_file, temp_dir, settings_dict, upc_lookup):
    ...
    def test_force_txt_extension(self, sample_edi_file, temp_dir, settings_dict, upc_lookup):
    ...
    def test_invoice_date_offset(self, sample_edi_file, temp_dir, settings_dict, upc_lookup):
...
class TestScansheetTypeAConverter:
    ...
    @patch("convert_to_scansheet_type_a.query_runner")
    def test_happy_path_conversion(self, mock_query, sample_edi_file, temp_dir, settings_dict, upc_lookup, mock_query_runner):
        """Test successful conversion with mocked database."""
        mock_query.return_value = mock_query_runner
    ...
    def test_interpret_barcode_string(self):
        ...
        result = converter_class.interpret_barcode_string(None, "123456789012")
        assert result == "123456789012"
        
        # Short UPC (needs padding)
    ...
    def test_interpret_barcode_string_errors(self):
        ...
        with pytest.raises(ValueError):
            converter_class.interpret_barcode_string(None, "12345678901234567890")
...
class TestSimplifiedCsvConverter:
    ...
    def test_include_headers_option(self, sample_edi_file, temp_dir, settings_dict, upc_lookup):
    ...
    def test_include_item_numbers_option(self, sample_edi_file, temp_dir, settings_dict, upc_lookup):
    ...
    def test_include_item_description_option(self, sample_edi_file, temp_dir, settings_dict, upc_lookup):
    ...
    def test_custom_column_order(self, sample_edi_file, temp_dir, settings_dict, upc_lookup):
...
class TestStewartsCustomConverter:
    ...
    @patch("convert_to_stewarts_custom.query_runner")
    def test_happy_path_conversion(self, mock_query, sample_edi_file, temp_dir, settings_dict, upc_lookup, mock_query_runner):
        """Test successful conversion with mocked database."""
        mock_query.return_value = mock_query_runner
    ...
    def test_convert_to_item_total_helper(self, sample_edi_file, temp_dir, settings_dict, upc_lookup):
...
class TestYellowdogConverter:
    ...
    @patch("convert_to_yellowdog_csv.utils.invFetcher")
    def test_complex_invoice_conversion(self, mock_invfetcher_class, complex_edi_file, temp_dir, settings_dict, upc_lookup):
        """Test conversion of complex EDI with multiple invoices."""
        mock_fetcher = MagicMock()
        mock_fetcher.fetch_cust_name.return_value = "Test Customer"
        mock_fetcher.fetch_po.return_value = "PO12345"
...
class TestConverterEdgeCases:
    ...
    def test_nonexistent_file_handling_scannerware(self, temp_dir, settings_dict, upc_lookup):
    ...
    def test_malformed_edi_handling_csv(self, malformed_edi_file, temp_dir, settings_dict, upc_lookup):
    ...
    def test_special_characters_in_description(self, temp_dir, settings_dict, upc_lookup):
...
class TestConverterPluginMetadata:
    ...
    def test_all_converters_have_plugin_id(self):
        ...
        for converter in converters:
            assert hasattr(converter, "PLUGIN_ID"), f"{converter.__name__} missing PLUGIN_ID"
    ...
    def test_all_converters_have_plugin_name(self):
        ...
        for converter in converters:
            assert hasattr(converter, "PLUGIN_NAME"), f"{converter.__name__} missing PLUGIN_NAME"
    ...
    def test_all_converters_have_config_fields(self):
        ...
        for converter in converters:
            assert hasattr(converter, "CONFIG_FIELDS"), f"{converter.__name__} missing CONFIG_FIELDS"
            assert isinstance(converter.CONFIG_FIELDS, list), f"{converter.__name__} CONFIG_FIELDS not a list"
    ...
    def test_config_fields_structure(self):
        ...
        for converter in converters_with_fields:
            for field in converter.CONFIG_FIELDS:
                assert "key" in field, f"{converter.__name__} config field missing 'key'"
                assert "label" in field, f"{converter.__name__} config field missing 'label'"
                assert "type" in field, f"{converter.__name__} config field missing 'type'"
