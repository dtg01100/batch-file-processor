...
class TestEDITweakBooleanCombinations:
    ...
    @pytest.mark.parametrize(
        "pad_arec,append_arec,force_txt,calc_upc,retail_uom,override_upc,split_tax,tweak_edi",
        [
    ...
    def test_boolean_combinations(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = tweaker.tweak(str(input_file), tmpdir, params, base_settings(), {})
    ...
    @pytest.mark.parametrize(
        "date_offset", [-365, -30, -14, -7, -1, 0, 1, 7, 14, 30, 365]
    )
    def test_invoice_date_offset_variations(
        self, base_parameters, base_settings, date_offset
    ):
    ...
    def test_upc_target_length_variations(
        self, base_parameters, base_settings, upc_length
    ):
    ...
    @pytest.mark.parametrize(
        "padding_pattern",
        ["", "0", "X", "12345", "0123456789", "ABCDEFGHIJ", "           "],
    )
    def test_upc_padding_pattern_variations(
        self, base_parameters, base_settings, padding_pattern
    ):
    ...
    def test_custom_date_format_variations(
        self, base_parameters, base_settings, date_format
    ):
    ...
    def test_override_upc_level_variations(
        self, base_parameters, base_settings, override_level
    ):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = tweaker.tweak(
                str(input_file), tmpdir, params, base_settings(), upc_dict
            )

            assert result is not None

    @pytest.mark.parametrize(
        "category_filter", ["ALL", "CAT1", "CAT1,CAT2", "CAT1,CAT2,CAT3,CAT4"]
    )
    def test_override_upc_category_filter_variations(
        self, base_parameters, base_settings, category_filter
    ):
    ...
    def test_a_record_padding_length_variations(
        self, base_parameters, base_settings, padding_length
    ):
    ...
    def test_zero_padding_length_with_padding_enabled(
        self, base_parameters, base_settings
    ):
    ...
    def test_custom_date_format_with_invalid_date(self, base_parameters, base_settings):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            input_file.write_text(
                "AVENDOR00000000000000000000100000\\n"
            )  # Date is 000000
