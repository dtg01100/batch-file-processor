...
class TestCSVConfigurationPlugin(unittest.TestCase):
    ...
    def test_static_properties(self):
        ...
        self.assertEqual(
            CSVConfigurationPlugin.get_description(),
            "Provides CSV format configuration options for EDI conversion",
        )
    ...
    def test_create_config(self):
        ...
        self.assertEqual(
            config.include_item_numbers, config_data["include_item_numbers"]
        )
        self.assertEqual(
            config.include_item_description, config_data["include_item_description"]
        )
        self.assertEqual(
            config.simple_csv_sort_order, config_data["simple_csv_sort_order"]
        )
        self.assertEqual(
            config.split_prepaid_sales_tax_crec,
            config_data["split_prepaid_sales_tax_crec"],
        )
    ...
    def test_serialize_config(self):
        ...
        self.assertEqual(
            serialized["include_item_numbers"], config.include_item_numbers
        )
        self.assertEqual(
            serialized["include_item_description"], config.include_item_description
        )
        self.assertEqual(
            serialized["simple_csv_sort_order"], config.simple_csv_sort_order
        )
        self.assertEqual(
            serialized["split_prepaid_sales_tax_crec"],
            config.split_prepaid_sales_tax_crec,
        )
