...
class TestFormGeneratorForPlugins(unittest.TestCase):
    ...
    def test_csv_config_field_types(self):
        ...
        self.assertEqual(
            fields["include_item_description"].field_type, FieldType.BOOLEAN
        )
        self.assertEqual(
            fields["split_prepaid_sales_tax_crec"].field_type, FieldType.BOOLEAN
        )
    ...
    def test_empty_config_validation(self):
        ...
        self.assertTrue(
            validation.success
        )  # No required fields, so empty should be valid
        self.assertEqual(len(validation.errors), 0)
...
class TestPluginUIRendering(unittest.TestCase):
    ...
    def test_create_widget_with_config(self, mock_builder):
        ...
        plugin = CSVConfigurationPlugin()
        test_config = {"include_headers": True, "filter_ampersand": True}
