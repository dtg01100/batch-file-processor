...
class TestCopyBackendOperations:
    ...
    def test_copy_backend_do_succeeds(self, process_parameters, settings_dict, source_file, dest_dir):
        ...
        with patch('builtins.open', mock_open(read_data="Test file content")):
            result = copy_backend.do(process_parameters, settings_dict, source_file, file_ops=mock_ops)
    ...
    def test_copy_backend_missing_file_raises(self, process_parameters, settings_dict, temp_dir):
    ...
    def test_copy_backend_with_mock_file_ops(self, process_parameters, settings_dict, source_file):
...
class TestFTPBackendOperations:
    ...
    def test_ftp_backend_with_mock_client(self, process_parameters, settings_dict, source_file):
        """Test FTP backend with mock client."""
        from backend.ftp_client import MockFTPClient
        
        mock_client = MockFTPClient()
        
        result = ftp_backend.do(process_parameters, settings_dict, source_file, ftp_client=mock_client)
        
        assert result is True
        assert len(mock_client.connections) > 0
        assert len(mock_client.logins) > 0

    def test_ftp_backend_file_sent(self, process_parameters, settings_dict, source_file):
        """Test that file is sent via FTP."""
        from backend.ftp_client import MockFTPClient
        
        mock_client = MockFTPClient()
        
        ftp_backend.do(process_parameters, settings_dict, source_file, ftp_client=mock_client)
        
        # Verify file was sent
        assert len(mock_client.files_sent) > 0

    def test_ftp_backend_creates_remote_directory(self, process_parameters, settings_dict, source_file):
    ...
    def test_ftp_backend_missing_file_raises(self, process_parameters, settings_dict, temp_dir):
...
class TestEmailBackendOperations:
    ...
    def test_email_backend_with_mock_client(self, process_parameters, settings, source_file):
        """Test email backend with mock client."""
        from backend.smtp_client import MockSMTPClient
        
        mock_client = MockSMTPClient()
        
        result = email_backend.do(process_parameters, settings, source_file, smtp_client=mock_client)
        
        assert result is True
        assert mock_client.ehlo_calls > 0

    def test_email_backend_sends_message(self, process_parameters, settings, source_file):
        """Test that email message is sent."""
        from backend.smtp_client import MockSMTPClient
        
        mock_client = MockSMTPClient()
        
        email_backend.do(process_parameters, settings, source_file, smtp_client=mock_client)
        
        # Verify send operations occurred
        assert mock_client.ehlo_calls > 0 or mock_client.send_message_calls > 0

    def test_email_backend_subject_placeholder(self, process_parameters, settings, source_file):
...
class TestBackendIntegration:
    ...
    def test_copy_and_email_backends(self, temp_dir, sample_file):
        ...
        email_result = email_backend.do(email_params, email_settings, sample_file, smtp_client=mock_smtp)
    ...
    def test_all_backends_with_same_file(self, temp_dir, sample_file):
        ...
        dest_dir = temp_dir / "copies"
        dest_dir.mkdir()
        copy_result = copy_backend.do({'copy_to_directory': str(dest_dir)}, {}, sample_file)
        ...
        ftp_result = ftp_backend.do(ftp_params, {}, sample_file, ftp_client=MockFTPClient())
        ...
        email_result = email_backend.do(email_params, email_settings, sample_file, smtp_client=MockSMTPClient())
...
class TestBackendErrorScenarios:
    ...
    def test_copy_permission_denied(self, temp_dir, sample_file):
        ...
        try:
            with pytest.raises(IOError):
                copy_backend.do({'copy_to_directory': str(readonly_dir)}, {}, sample_file)
        finally:
    ...
    def test_ftp_connection_failure(self, sample_file):
        ...
        with pytest.raises(Exception):
            ftp_backend.do(
                {'ftp_server': 'invalid', 'ftp_port': 21, 'ftp_username': 'x', 'ftp_password': 'x', 'ftp_folder': '/'},
                {},
                sample_file,
    ...
    def test_email_auth_failure(self, sample_file):
        ...
        try:
            result = email_backend.do(
                ...
                {'email_address': 'sender@example.com', 'email_smtp_server': 'smtp.example.com', 'smtp_port': 587},
                sample_file,
