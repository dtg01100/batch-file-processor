...
if __name__ == "__main__":
    ...
    def make_users_list():
        ...
        active_folder_dict_list = database_obj_instance.folders_table.find(
            folder_is_active="True"
        )
        inactive_folder_dict_list = database_obj_instance.folders_table.find(
        ...
        if folder_filter != "":
            ...
            fuzzy_filter = list(
                thefuzz.process.extractWithoutOrder(
                    folder_filter, folder_alias_list, score_cutoff=80
                )
            )
            ...
            for entry in fuzzy_filtered_alias:
                pre_filtered_folder_dict_list.append(
                    copyf(
                        database_obj_instance.folders_table.find(order_by="alias"),
                        "alias",
                        entry,
                    )
                )
                pre_filtered_active_folder_dict_list.append(
                    copyf(
                        database_obj_instance.folders_table.find(
                            folder_is_active="True"
                        ),
                        "alias",
                        entry,
        ...
        if len(list(filtered_folder_dict_list)) == 0:
            no_active_label = tkinter.ttk.Label(
                active_users_list_frame, text="No Active Folders"
            )
            no_active_label.pack(fill=tkinter.BOTH, expand=1, padx=10)
            no_inactive_label = tkinter.ttk.Label(
                inactive_users_list_frame, text="No Inactive Folders"
            )
        ...
        if database_obj_instance.folders_table.count(folder_is_active="True") != 0:
            ...
            if len(list(active_folder_alias_list)) != 0:
                active_folder_edit_length = len(
                    max(list(active_folder_alias_list), key=len)
                )
        ...
        if database_obj_instance.folders_table.count(folder_is_active="False") != 0:
            ...
            if len(list(inactive_folder_alias_list)) != 0:
                inactive_folder_edit_length = len(
                    max(list(inactive_folder_alias_list), key=len)
                )
        ...
        for folders_name in filtered_folder_dict_list:
            if str(folders_name["folder_is_active"]) != "False":
                active_folder_button_frame = tkinter.ttk.Frame(
                    active_users_list_frame.interior
                )
                tkinter.ttk.Button(
                ...
                inactive_folder_button_frame = tkinter.ttk.Frame(
                    inactive_users_list_frame.interior
                )
                tkinter.ttk.Button(
                ...
                tkinter.ttk.Button(
                    ...
                    text="Edit: " + folders_name["alias"] + "...",
                    command=lambda name=folders_name["id"]: edit_folder_selector(name),
                    width=inactive_folder_edit_length + 6,
        ...
        if (
            len(list(filtered_folder_dict_list))
            != database_obj_instance.folders_table.count()
        ):
            folders_count_label = tkinter.ttk.Label(
                search_frame,
                text=(
                    str(len(list(filtered_folder_dict_list)))
                    + " of "
                    + str(database_obj_instance.folders_table.count())
                    + " shown"
                ),
            )
        ...
        tkinter.ttk.Separator(
            active_users_list_container, orient=tkinter.HORIZONTAL
        ).pack(fill=tkinter.X)
        inactive_users_list_label.pack(pady=5)
        tkinter.ttk.Separator(
            inactive_users_list_container, orient=tkinter.HORIZONTAL
        ).pack(fill=tkinter.X)
        active_users_list_frame.pack(
            fill=tkinter.BOTH, expand=tkinter.TRUE, anchor=tkinter.E, padx=3, pady=3
        )
        inactive_users_list_frame.pack(
            fill=tkinter.BOTH, expand=tkinter.TRUE, anchor=tkinter.E, padx=3, pady=3
        )
        active_users_list_container.pack(
            side=tkinter.RIGHT, expand=tkinter.TRUE, fill=tkinter.Y
        )
        inactive_users_list_container.pack(
        ...
        tkinter.ttk.Separator(users_list_frame, orient=tkinter.HORIZONTAL).pack(
            side=tkinter.BOTTOM, fill=tkinter.X
        )
        scrollable_lists_frame.pack(
    ...
    def update_folder_alias(
        folder_edit,
    ):  # update folder settings in database with results from EditDialog
    ...
    def delete_folder_entry_wrapper(folder_to_be_removed, alias):
        ...
        if askyesno(
            message="Are you sure you want to remove the folder " + alias + "?"
        ):
            delete_folder_entry(folder_to_be_removed)
            refresh_users_list()
            set_main_button_states()

    def graphical_process_directories(
        ...
        if missing_folder is True:
            ...
            if folders_table_process.count(folder_is_active="True") > 0:
                doingstuffoverlay.make_overlay(
                    parent=root, overlay_text="processing folders..."
                )
    ...
    def process_directories(folders_table_process):
        ...
        if check_logs_directory() is False or log_folder_creation_error is True:
            if args.automatic is False:
                ...
                while (
                    check_logs_directory() is False
                ):  # don't let user out unless they pick a writable folder, or cancel
                    if askokcancel(
                        "Error",
                        ...
                        EditSettingsDialog(
                            root,
                            database_obj_instance.oversight_and_defaults.find_one(id=1),
                        )
                    else:
                        # the logs must flow. aka, stop here if user declines selecting a new writable log folder
                        showerror(
                            parent=root, message="Can't write to log directory, exiting"
                        )
                        raise SystemExit
            else:
                try:
        ...
        run_log_path = str(
            run_log_path
        )  # convert possible unicode path to standard python string to fix weird path bugs
        ...
        with open(run_log_full_path, "wb") as run_log:
            ...
            try:
                run_error_bool, run_summary_string = dispatch.process(
                    database_obj_instance.database_connection,
                    folders_table_process,
                    run_log,
            ...
            try:
                ...
                for log in database_obj_instance.emails_table.all():
                    ...
                    if os.path.isfile(os.path.abspath(log["log"])):
                        ...
                        if (
                            total_size > 9000000
                            or database_obj_instance.emails_table_batch.count() >= 15
                        ):
                            batch_log_sender.do(
                                settings_dict,
                                reporting,
                                database_obj_instance.emails_table_batch,
                ...
                if (
                    skipped_files > 0
                ):  # log any skipped files, try to send that error in the run log
                    batch_number += 1
                    emails_count += 1
                    email_errors.write(
                        "\r\n\r\n" + str(skipped_files) + " emails skipped"
                    )
                    email_errors_log_name_constructor = (
                        "Email Errors Log "
                        + str(time.ctime()).replace(":", "-")
                        + ".txt"
                    )
                    email_errors_log_full_path = os.path.join(
                        run_log_path, email_errors_log_name_constructor
                    )
                    with open(
                    ...
                    database_obj_instance.emails_table_batch.insert(
                        dict(
                            log=email_errors_log_full_path,
    ...
    def automatic_process_directories(automatic_process_folders_table):
        if automatic_process_folders_table.count(folder_is_active="True") > 0:
            ...
            try:
                tkinter.ttk.Label(root, text="Running In Automatic Mode...").pack(
                    side=tkinter.TOP
                )
    ...
    def remove_inactive_folders():  # loop over folders and safely remove ones marked as inactive
        ...
        if database_obj_instance.folders_table.count(folder_is_active="False") > 0:
            users_refresh = True
        folders_total = database_obj_instance.folders_table.count(
            folder_is_active="False"
        )
        folders_count = 0
        doingstuffoverlay.make_overlay(
        ...
        for folder_to_be_removed in database_obj_instance.folders_table.find(
            folder_is_active="False"
        ):
            folders_count += 1
            doingstuffoverlay.update_overlay(
    ...
    def mark_active_as_processed(master, selected_folder=None):
        ...
        if selected_folder is None:
            for row in database_obj_instance.folders_table.find(
                folder_is_active="True"
            ):
    ...
    def processed_files_popup():
        ...
        def folder_button_pressed(name):
            ...
            export_button = tkinter.ttk.Button(
                ...
                command=lambda: export_processed_report(
                    name, processed_files_output_folder
                ),
            )
        ...
        for folders_name, folder_alias in sorted_folder_list:
            folder_row = database_obj_instance.processed_files.find_one(
    ...
    open_folder_button = tkinter.ttk.Button(
        options_frame, text="Add Directory...", command=select_folder
    )
    open_multiple_folder_button = tkinter.ttk.Button(
        options_frame, text="Batch Add Directories...", command=batch_add_folders
    )
    default_settings = tkinter.ttk.Button(
        options_frame, text="Set Defaults...", command=set_defaults_popup
    )
    edit_reporting = tkinter.ttk.Button(
        options_frame,
        text="Edit Settings...",
        command=lambda: EditSettingsDialog(
            root, database_obj_instance.oversight_and_defaults.find_one(id=1)
        ),
    )
    process_folder_button = tkinter.ttk.Button(
        options_frame,
    ...
    allow_resend_button = tkinter.ttk.Button(
        options_frame,
        text="Enable Resend...",
    ...
    maintenance_button = tkinter.ttk.Button(
        options_frame, text="Maintenance...", command=maintenance_functions_popup
    )
    processed_files_button = tkinter.ttk.Button(
        options_frame, text="Processed Files Report...", command=processed_files_popup
    )
    ...
    tkinter.ttk.Separator(options_frame, orient=tkinter.HORIZONTAL).pack(
