...
| 2026-08-12 | Project Owner | Open questions 2 + 3 resolved (`TBD`): runner writes per-folder raw error artifacts + `error_file` column + `GET /api/errors/file` download; `folder_counts` in `/api/errors` + count labels in the Errors-card folder dropdown |
