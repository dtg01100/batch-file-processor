...
function PipelineReadExcelNode({ data, selected, onUpdate }) {
  const sheetName = data.sheetName || 'Sheet1';
  const hasHeader = data.hasHeader !== false;
  
  return (
    <div 
      className={`pipeline-node read-excel ${selected ? 'selected' : ''}`}
      style={{ 
      ...
      title="Double-click to configure Excel reading"
    >
      <Handle
        type="target"
      ...
      <div className="node-content">
        ...
        <div className="node-text">
          <div className="node-label">{data.label || 'Read Excel'}</div>
          <div className="node-summary">
            <span className="excel-tag">{sheetName}</span>
            <span className="excel-tag">{hasHeader ? 'header' : 'no header'}</span>
          </div>
        </div>
      </div>
      ...
      <style>{`
        ...
        .excel-tag { padding: 2px 6px; background: #E8F5E9; border-radius: 4px; font-size: 9px; color: #388E3C; }
...
export default memo(PipelineReadExcelNode);
