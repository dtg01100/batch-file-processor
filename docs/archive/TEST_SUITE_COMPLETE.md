...
- ⚡ **Minimal I/O operations**: Only when necessary
- ⚡ **Real test databases**: Using isolated temp copies
- ⚡ **No display server**: Mocked only for PyQt6 widgets

**Note:** Following project guidelines, we minimize mocking and prefer real
implementations wherever practical. Mocks are used only for:
- PyQt6 UI components (require display server)
- External services (FTP/SMTP backends)
