...
**Testing Philosophy:** Following project guidelines, tests minimize mocks and fakes,
prefering real implementations with isolated fixtures (temp directories, test databases).
Mocks are used only for external services (FTP/SMTP), UI components requiring display
servers, or truly expensive operations.
