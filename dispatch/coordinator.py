...
class DispatchCoordinator:
    ...
    def __init__(self, 
        ...
        self.db_manager = DBManager(database_connection, processed_files, folders_database)
    ...
    def _create_overlay_updater(self):
        ...
        def update_overlay(overlay_text, dispatch_folder_count, folder_total, 
                          dispatch_file_count, file_total, footer):
    ...
    def process(self) -> Tuple[bool, str]:
        ...
        for parameters_dict in self.context.parameters_dict_list:
            ...
            self.update_overlay("processing folder...\n\n", folder_count, folder_total_count, 
                              self.context.file_count, 0, "")
            
            if os.path.isdir(parameters_dict['folder_name']) is True:
                print(f"processing folder {parameters_dict['folder_name']}, aliased as {parameters_dict['alias']}")
                ...
                if parameters_dict['folder_name'] != hash_thread_return_dict['folder_name']:
                    raise ValueError(f"desync between current folder {parameters_dict['folder_name']} and current entry in hashed queue {hash_thread_return_dict['folder_name']}")
                
                # Process files in folder
                folder_errors = self._process_folder(
                    parameters_dict, files, filtered_files, folder_count, folder_total_count
                )
        ...
        edi_validator_error_report_string = ", has EDI validator errors" if self.context.global_edi_validator_error_status else ""
    ...
    def _write_validation_report(self) -> str:
        ...
        import datetime
        validator_log_name = f"Validator Log {datetime.datetime.now().isoformat().replace(':', '-')}.txt"
        ...
        with open(validator_log_path, 'wb') as validator_log_file:
            validator_log_file.write(self.context.edi_validator_errors.getvalue().encode())
    ...
    def _create_hash_thread(self, temp_processed_files_list: List[Dict[str, Any]]) -> threading.Thread:
        ...
        def hash_thread_target():
            with concurrent.futures.ProcessPoolExecutor() as hash_executor:
                for counter, entry_dict in enumerate(self.context.parameters_dict_list):
                    ...
                    print(f"Generating file hashes {counter + 1} of {len(self.context.parameters_dict_list)} ({entry_dict['folder_name']})")
                    ...
                    # Queue results
                    self.context.hash_thread_return_queue.put(dict(
    ...
    def _process_folder(self, parameters_dict: Dict[str, Any], 
                       files: List[str], 
                       filtered_files: List[Tuple],
                       folder_count: int,
                       folder_total_count: int) -> bool:
        """
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for result in executor.map(
                lambda idx: self._process_single_file(idx, parameters_dict, filtered_files, folder_errors_log),
                [x[0] for x in filtered_files]
            ):
                errors, original_filename, file_checksum, return_log, return_error_log = result
                ...
                if not file_count == file_count_total:
                    ...
                    self.update_overlay("processing folder... (updating database records)\n\n", folder_count,
                                       folder_total_count, len(processed_files_insert_list) + 1, file_count_total,
    ...
    def _write_folder_errors_report(self, parameters_dict: Dict[str, Any], errors: str):
        ...
        if not os.path.exists(self.errors_folder['errors_folder']):
            record_error.do(self.run_log, folder_error_log_name_full_path, "Base errors folder not found",
        ...
        if not os.path.exists(folder_errors_dir):
            record_error.do(self.run_log, folder_error_log_name_full_path, "Error folder Not Found",
                          parameters_dict['folder_name'], "Dispatch Error Logger")
            try:
                os.mkdir(folder_errors_dir)
            except IOError:
                record_error.do(self.run_log, folder_error_log_name_full_path, "Error creating errors folder",
                              parameters_dict['folder_name'], "Dispatch Error Logger")
                folder_error_log_name_full_path = os.path.join(self.run_log_directory, folder_error_log_name_constructor)
        
        with open(folder_error_log_name_full_path, 'wb') as folder_errors_log_write:
            utils.do_clear_old_files(folder_errors_dir, 500)
            folder_errors_log_write.write(f"Program Version = {self.version}\r\n\r\n".encode())
            folder_errors_log_write.write(errors.encode())
    
    def _process_single_file(self, file_index: int, parameters_dict: Dict[str, Any], 
                            filtered_files: List[Tuple], folder_errors_log) -> Tuple:
        """
        ...
        with tempfile.TemporaryDirectory() as file_scratch_folder:
            input_filename = os.path.join(
                os.path.abspath(parameters_dict['folder_name']), 
                str(input_filename)
            )
            ...
            # Validate EDI
            valid_edi_file = True
            if (parameters_dict['process_edi'] == "True" or
                parameters_dict['tweak_edi'] or
                parameters_dict['split_edi'] or
                parameters_dict['force_edi_validation']):
                print(input_filename)
                validation_result = self.edi_validator.validate_file(input_filename, process_original_filename)
                if validation_result.has_errors:
                    valid_edi_file = False
                    if validation_result.has_minor_errors:
                        self.context.edi_validator_errors.write(f"\r\nErrors for {process_original_filename}:\r\n")
                        self.context.edi_validator_errors.write(validation_result.error_message)
                        self.context.global_edi_validator_error_status = True
            
            # Process EDI
            split_edi_list = self._process_edi(
                input_filename, parameters_dict, valid_edi_file, file_scratch_folder,
            ...
            for output_send_filename, filename_prefix, filename_suffix in split_edi_list:
                ...
                if not skip_file:
                    errors = self._send_file(
                        output_send_filename, parameters_dict, 
                        filename_prefix, filename_suffix,
                        process_files_log, process_files_error_log,
                        input_filename, valid_edi_file, file_scratch_folder
                    )
    ...
    def _process_edi(self, input_filename: str, parameters_dict: Dict[str, Any],
                    valid_edi_file: bool, scratch_folder: str,
                    process_files_log: List, process_files_error_log: List) -> List[Tuple]:
        """
        ...
        if parameters_dict['split_edi'] and valid_edi_file:
            ...
            try:
                ...
                if len(split_edi_list) > 1:
                    process_files_log.append(f"edi file split into {len(split_edi_list)} files\r\n\r\n")
    ...
    def _send_file(self, output_send_filename: str, parameters_dict: Dict[str, Any],
                  filename_prefix: str, filename_suffix: str,
                  process_files_log: List, process_files_error_log: List,
                  input_filename: str, valid_edi_file: bool,
                  scratch_folder: str) -> bool:
        """
        ...
        if os.path.exists(output_send_filename):
            if parameters_dict['process_edi'] != "True" and not errors:
                output_filename = os.path.join(scratch_folder, os.path.basename(rename_file))
                ...
                if not errors:
                    ...
                    try:
                        ...
                        if parameters_dict['process_edi'] == "True":
                            print(f"Converting {output_send_filename} to {parameters_dict['convert_to_format']}")
                            process_files_log.append(f"Converting {output_send_filename} to {parameters_dict['convert_to_format']}\r\n\r\n")
                            output_send_filename = EDIConverter.convert_edi(
                                output_send_filename, output_filename, 
                                self.settings, parameters_dict, self.context.upc_dict
                            )
                            print("Success")
                        ...
                        if parameters_dict['tweak_edi'] is True:
                            ...
                            process_files_log.append(f"Applying tweaks to {output_send_filename}\r\n\r\n")
        ...
        send_results = self.send_manager.send_file(output_send_filename, parameters_dict, self.settings)
