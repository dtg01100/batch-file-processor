...
class ErrorLogger:
    ...
    def log_error(self, error_message: str, filename: str, module: str):
        ...
        """
        record_error.do(self.run_log, self.folder_errors_log, error_message, filename, module)
    
    def log_folder_error(self, error_message: str, folder_name: str, module: str = "Dispatch"):
        """
    ...
    def log_file_error(self, error_message: str, filename: str, module: str = "Dispatch"):
        """
...
class ErrorHandler:
    ...
    def write_folder_errors_report(self, folder_name: str, folder_alias: str, version: str) -> str:
        """
        ...
        Returns:
            Path to the generated report file
        """
        cleaned_alias_string = ReportGenerator._clean_filename(folder_alias)
        log_name = f"{cleaned_alias_string} errors.{datetime.datetime.now().isoformat().replace(':', '-')}.txt"
        log_path = os.path.join(self.errors_folder['errors_folder'], os.path.basename(folder_name), log_name)
        
        # Ensure errors directory exists
        if not os.path.exists(self.errors_folder['errors_folder']):
            self.log_folder_error("Base errors folder not found", folder_name, "Dispatch Error Logger")
        ...
        if not os.path.exists(folder_errors_dir):
            self.log_folder_error("Error folder not found", folder_name, "Dispatch Error Logger")
            try:
                os.mkdir(folder_errors_dir)
            except IOError as e:
                self.log_folder_error(f"Error creating errors folder: {str(e)}", folder_name, "Dispatch Error Logger")
                log_path = os.path.join(self.run_log_directory, log_name)
        
        report_content = self.report_generator.generate_processing_report(self.logger.get_errors(), version)
