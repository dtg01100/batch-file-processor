...
class EDISplitter:
    ...
    def split_edi(input_file: str, scratch_folder: str, parameters_dict: dict) -> List[Tuple[str, str, str]]:
        ...
        """
        try:
            split_edi_list = utils.do_split_edi(input_file, scratch_folder, parameters_dict)
...
class EDIConverter:
    ...
    @staticmethod
    def convert_edi(input_file: str, output_file: str, 
                   settings: dict, parameters_dict: dict, upc_dict: dict) -> str:
        """
...
class EDITweaker:
    ...
    @staticmethod
    def tweak_edi(input_file: str, output_file: str, 
                 settings: dict, parameters_dict: dict, upc_dict: dict) -> str:
        """
        ...
        return edi_tweaks.edi_tweak(input_file, output_file, settings, parameters_dict, upc_dict)
...
class EDIProcessor:
    ...
    @staticmethod
    def process_edi(input_file: str, parameters_dict: dict, 
                   settings: dict, upc_dict: dict, scratch_folder: str) -> List[Tuple[str, str, str]]:
        """
        ...
        if parameters_dict['split_edi']:
            processed_files = EDISplitter.split_edi(input_file, scratch_folder, parameters_dict)
    ...
    @staticmethod
    def _process_single_file(input_file: str, parameters_dict: dict, 
                           settings: dict, upc_dict: dict, scratch_folder: str) -> str:
        """
        ...
        """
        # Copy file to scratch folder for processing
        ...
        if parameters_dict['process_edi'] == "True":
            converted_file = os.path.join(scratch_folder, f"converted_{os.path.basename(temp_file)}")
            temp_file = EDIConverter.convert_edi(temp_file, converted_file, settings, parameters_dict, upc_dict)
        
        # Apply tweaks if configured
        if parameters_dict['tweak_edi'] is True:
            tweaked_file = os.path.join(scratch_folder, f"tweaked_{os.path.basename(temp_file)}")
            temp_file = EDITweaker.tweak_edi(temp_file, tweaked_file, settings, parameters_dict, upc_dict)
