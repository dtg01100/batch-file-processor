...
class UIProtocol(Protocol):
    ...
    def update_email_progress(
        self, batch_number: int, email_count: int, total_emails: int
    ) -> None:
...
class LogSender:
    ...
    def send_log_file(self, log_path: str, recipients: list[str], subject: str) -> bool:
    ...
    def send_batch_logs(
        ...
        for i, log in enumerate(logs):
            self.ui.update_email_progress(
                batch_number=batch_number, email_count=i + 1, total_emails=len(logs)
            )
