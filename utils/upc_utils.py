"""
UPC and barcode utilities.

This module provides functions for calculating check digits and converting
between different UPC barcode formats.
...
def calc_check_digit(value: str) -> int:
    """Calculate check digit for UPCA and UPCE barcodes.

    Args:
        value: The barcode value (without check digit)

    Returns:
        The calculated check digit (0-9)

    Example:
        calc_check_digit("041800000265") → 5
    """
    check_digit = 0
    odd_pos = True
...
def convert_UPCE_to_UPCA(upce_value: str) -> str:
    """Convert UPCE barcode to UPCA format.

    Args:
        upce_value: UPCE barcode (6, 7, or 8 characters)

    Returns:
        UPCA barcode (12 characters + check digit), or False if invalid

    Example:
        convert_UPCE_to_UPCA("04182635") → "0418000002655"
    """
    if len(upce_value) == 6:
